
<!DOCTYPE html>

<head>
    <title>Donate</title>

    <link rel="stylesheet" type="text/css" href="Style/style.css">

</head>

<body id="fullImage">

    <!-- Shiriti nagivimit -->
    <header>
        <nav class="navbar">
            <div class="leftside">

                <div class="logoMove">
                    <img class="logo" src="Fotot/logo.png" alt="logo">


                    <ul class="ulnav">
                        <li class="navlist"><a class="ula" href="index.php">Home</a></li>
                        <li class="navlist">
                            <a class="ula" href="aboutus.php">About Us</a>
                        </li>
                        <li class="navlist">
                            <a class="ula" href="contact.php">Contact</a>
                        </li>
                    </ul>
                </div>

            </div>

            <ul class="ulnav">

                <a class="ula" id="pad" href="register.php">Register</a>
                </li>
                <li class="navlist">
                    <a class="ula" id="pad" href="login.php">Login</a>
                </li>
            </ul>


        </nav>
    </header>
    <!-- Main Content -->
    <div class="mainimg">

        <img class="main" id="SliderImages" src="" alt="">

    </div>

    <div class="heading">

        <h1>"We make a living by what we get, but we make a life by what we give."</h1><br><br>

    </div>
    <main id="mainColums">


        <div class="leftcolum">

            <img src="Fotot/clothes.jpg" class="clothes"><br><br>
            <a href="donateclothes.php">

                <button type="button" class="button"> <a class="btnA" href="donateclothes.php"> Donate Clothes
                    </a></button><br><br>
            </a>


        </div>

        <div class="rightcolum">
            <img class="money" src="Fotot/money.jpg"><br><br>

            <a href="donatemoney.php">

                <button type="button" class="button"> <a class="btnA" href="donatemoney.php">Donate
                        Money</a></button><br><br>
            </a>


        </div>
    </main>
    <br><br><br>
    <!-- Footeri -->

    <footer>
        <div class="footer">

            <div class="footercolum1">

                <h4>Our Website</h4> <br>
                <p>"HelpME" is a website for people that want to help<br> others with clothes and money, after they
                    donate<br> we send the supplies to people in need of them. </p>


            </div>

            <div class="footercolum2">

                <h4>Navigation</h4>

                <ul class="ulfooter">

                    <li class="lifooter"><a href="index.php">
                            <p> Home </p> </a> 
                    </li>
                    <li class="lifooter"><a href="aboutus.php">
                            <p>About Us</p>
                        </a></li>
                    <li class="lifooter"> <a href="contact.php">
                            <p> Contact </p>
                        </a></li>
                    <li class="lifooter"> <a href="register.php">
                            <p> Register </p>
                        </a></li>
                    <li class="lifooter"> <a href="login.php">
                            <p> Login </p>
                        </a></li>

                </ul>

            </div>

            <div class="logofooter">

                <img class="logolast" src="Fotot/logo.png">

            </div>

        </div>
    </footer>




    <script src="Script/home.js"></script>
</body>


</html>